...
@if(count($content)>0)
    ...
    <!-- <div class="table-responsive"> -->
                        ...
                        <tr>
                          ...
                          <td> {{$conten->type}}</td>
                          <td> {{$conten->title}}</td>
                            <td> {{$conten->description}}</td> 
                            <td><img src="{{asset('/imgs/'.$conten->link)}}" alt=""> </td>
                            <td>
            <form method="POST" action="{{route('coach.delet.content',$conten->id)}}">
              @csrf
    @method('POST')
...
<td >
<form method="POST" action="{{route('coach.edit.content',$conten->id)}}">
@csrf
@method('POST')
<button type="submit" class="btn btn-success">Edit</button>
          ...
          <h3 style="color: #555; font-size: 24px;">There Are No Content</h3>
        </div>
                      @endif
                    </div>
                    </div>	
